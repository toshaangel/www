 ...
 * \clr[ColorIndex] // Измените цвет текста с помощью палитры цветов, определенной в параметрах этого плагина.
// ColorIndex: Индекс цвета
\olc[ColorIndex] // Измените цвет контура текста с помощью палитры цветов, определенной в этом плагине
// Параметры.
// ColorIndex: Индекс цвета
\olw[value] // Изменение ширины контура текста.
// значение: Ширина внешней линии

\fc[faceName, faceIndex] // Измените текущую грань сообщения на указанную грань.
// faceName: Имя файла лица
// faceIndex: Индекс лица
\actfc[actorID] // Измените текущую грань сообщения на указанную грань актера.
// actorID: Индекс актера
\prtfc[value] // Измените текущую грань сообщения на грань актора указанной позиции стороны.
// значение: Партийная позиция

\b[state] // Отображает следующий текст жирным шрифтом.
// государство: Смелое государство
// 1 - Включить жирный шрифт | 0 - Отключить жирный шрифт
\it[state] // Отображает следующий текст курсивом.
// государство: Курсивное состояние
// 1 - Включить курсив | 0 - Отключить курсив


\sfx[state] // Управление звуковым эффектом сообщения.
// государство: Состояние SFX
// on - Включить SFX | off - Отключить SFX
\sfx[id] // Изменение звукового эффекта текущего сообщения.
// id: Индекс SFX

\isf[state] // Управление входным шоу быстро.
// государство: Входные данные показывают быстрое состояние
// on - Включить ISF | off - Отключить ISF
Информация о данных: Элементы:
// Предметы
\itn[id] // Отображает имя указанного элемента id.
\iti[id] // Отображает значок указанного элемента id.
\itq[id] // Отображает текущее количество партии указанного элемента id.
// id: Идентификатор элемента

// Оружие
\wpn[id] // Отображает имя указанного идентификационного оружия.
\wpi[id] // Отображает значок указанного идентификационного оружия.
\wpq[id] // Отображает текущее количество партии указанного идентификационного оружия.
// id: Идентификатор оружия

// Доспехи
\arn[id] // Отображает имя указанного идентификатора брони.
\ari[id] // Отображает значок указанного идентификатора брони.
\arq[id] // Отображает текущее количество партии указанной брони id.
// id: Идентификатор брони

// Навыки
\skn[id] // Отображает имя указанного навыка id.
\ski[id] // Отображает значок указанного навыка id.
// id: Идентификатор навыка
Команды режима воздушного шара:
\tgt[this] // Измените цель воздушного шара сообщения на текущее событие.

\tgt[id] // Измените цель воздушного шара сообщения.
// id: Целевой идентификатор
// 0 - игрок
// 1 > - событие

\tgt[PPOs] // Измените цель воздушного шара сообщения на актора указанного
// партийная позиция. Если вы используете плагин Akea Animated Battle System
// он также будет работать во время сражений,
// Pos: Позиция партии

\tgt[aId] // Измените цель воздушного шара сообщения на указанный идентификатор субъекта. Если
// вы используете плагин Akea Animated Battle System, он также будет работать
// во время сражений,
// Id: Индекс актера

\tgt[eId] // Измените цель воздушного шара сообщения на указанную боевую цель
// id. Работает только в том случае, если вы используете плагин Akea Animated Battle System.
// Id: Индекс боевой цели

\spp // Show the balloon pop.

\hpp // Спрячьте поп-баллон.
