 ...
 * @target MV MZ
 * @url http://kdworkshop.net/map-inventory/
 * @plugindesc v1.8 [PRO] - Visual Map Inventory
 * 
 * @help
 * === === === === === === === === === === === === === === === === ===
 * ! This plugin is separated part of Alpha ABS plugin
 * Guides and extra resources you can find at: http://kdworkshop.net/map-inventory/
 * 
 ...
 *      VisualChest NAME - open visual chest, where NAME - chest name
 *          * VisualChest SomeChest * (for example)
...
// Generated by CoffeeScript 2.5.1
// * Эти вызовы используются вместе команд плагина
(function() {
...
(function() {
  ...
  //TODO: Когда через команду меняется количество Locked, перезагрузить инвентарь (если открыт)
  //TODO: Магазин?
  //TODO: Проверка чтобы страницы нельзя было листат, если предметов много
  //TODO: Добавление предметов через команду плагина??? Как быть?
  //TODO: Take All?
  // * Можно сделать параметр, чтобы запретить или оставить (будет в обычном инвентаре)
  // * Или увеличить слот? (если команда события дать предмет)
  _.getMapInventoryUnlockedCount = function(categoryIndex) {
    var db;
    db = this.miGetLockSystem();
    //TODO: parameter, if not, -1 - all
    //TODO: parameter variable for unlocked count
    //TODO: initialization, if null
    return db[categoryIndex];
  };
  _.miGetLockSystem = function() {
    if (this._miLockCounters == null) {
      //TODO: from variables?
      // if variable == 0, then 5
      // if variable < 0, then 0
      // if variable > 0, then variable
      this._miUnLockCounters = [4, 3, 2];
    }
    return this._miUnLockCounters;
  };
})();
...
(function() {
  ...
  _.initialize = function() {
    ...
    return PKD_MI.Parameters = new PKD_MI.LIBS.Parameters();
  };
})();
...
(function() {
  ...
  Sprite_ItemExtraInfo = class Sprite_ItemExtraInfo extends KDCore.Sprite {
    ...
    _drawText() {
      ...
      return this._description.drawTextExWithWordWrap(this._extraInfoText, 0, 0, this.extraInfo.info.size.w);
    }
...
(function() {
  ...
  Sprite_MapInvCell = class Sprite_MapInvCell extends KDCore.Sprite {
    ...
    _createLockCellSprite() {
      ...
      this._lockCellSpr = new Sprite(); //new Sprite(ImageManager.loadPKDMI("InventorySlot_locked"))
      this._lockCellSpr.visible = false;
      return this.addChild(this._lockCellSpr);
    }

    //?UPD18
    prepareForCategory(categoryIndex) {
...
(function() {
  ...
  Sprite_MapInvHelp = class Sprite_MapInvHelp extends KDCore.Sprite {
    ...
    drawDesc() {
      ...
      // * why + 150 ???
      return this._description.drawTextExWithWordWrap(text, 0, 0, this.settings.descriptionWindow.width + 150);
    }
...
//Plugin PKD_MapInventory automatic build by MVPluginBuilder 1.7 01.02.2021
