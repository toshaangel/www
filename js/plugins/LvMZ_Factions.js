...
if (!Imported['LvMZCore']) {
	...
	Game_Interpreter.prototype.actor = function(index) {
...
/*:
 * @target MZ
 * @plugindesc [v1.0] Core Plugin - Required for other plugins interacting
 * with factions, relationship, or titles (ex: LvMZ_Economy.js)
 * @author LordValinar
 ...
 *     Type: Faction  -or-  Race  -or-  Relation  -or-  Title 
 ...
 *   For the 4 plugin commands: Set Faction, Set Race, Add Title and 
 *     Remove Title, you can substitute the faction names with a 
 *     variable. (Example: \v[13])
 *
 ...
 * this.factionsMatch(object)                   - Returns a boolean
...
Game_Actor.prototype.initFactionSettings = function() {
	...
	const tag = /<(FACTION||RACE||TITLE):\s([^>]*)>/i;
...
Game_Event.prototype.meetsConditions = function(page) {
	...
	const tagSw = /\[(FACTION||RACE||RELATION||TITLE)\](.*)/i;
...
Game_Event.prototype.initNPC = function(page) {
	...
	const setupTag = /<INIT:\s(FACTION||RACE||RELATION||TITLE)\s([^>]*)\s(\d+)>/i;
...
LordV_Factions.prototype.addTitle = function(object, name) {
	if (this.list('title').includes(name)) {
		if (object._titles.includes(name)) return; // already have it
